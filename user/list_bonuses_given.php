<!-- List of all bonuses that this user has given. -->
<?php
...
// ===================== SQL Queries Section =====================
$sql_query = "SELECT  bonus.bonus_id, bonus.amount,  bonus.receiver_wrk_id, wrk_name, year_id
FROM  bonus JOIN worker ON receiver_wrk_id = wrk_id
		JOIN yearly_bonus ON yearly_bonus.bonus_id = bonus.bonus_id
WHERE bonus.giver_wrk_id = $my_wrk_id
";
// ===================== SQL Queries Section =====================
$bonus_rows = array();
$statement = $sql_query;
...
if (!empty($bonus_rows)) {
    ...
    <table>
        ...
        for ($i = 0; $i < count($bonus_rows); $i++) {
            ...
            <tr>
                ...
                <td><?php print $bonus_rows[$i]['receiver_wrk_id']; ?></td>
    ...
    ?>
    <p>You don't have any given bonuses yet!</p>
    <?php
}
?>
